...
Create a BigQuery view that transforms the standard GCP billing export into FOCUS format. It's recommended to refer to the official Google documentation:
- https://docs.cloud.google.com/billing/docs/how-to/create-billing-account#set-up-billing-project
- https://docs.cloud.google.com/billing/docs/how-to/export-data-bigquery-setup
- https://cloud.google.com/resources/google-cloud-focus

## Prerequisites

- GCP Detailed Usage Export enabled (`gcp_billing_export_resource_v1_*`)
- GCP Cloud Pricing Export enabled (`cloud_pricing_export`)

## Create the View

Run this SQL in BigQuery, replacing the highlighted paths with your actual table paths:
...
usage_cost_data AS (
  SELECT
    *,
    ...
    ARRAY(
      (SELECT AS STRUCT key, value, "label" AS x_type, FALSE AS x_inherited, "n/a" AS x_namespace FROM UNNEST(labels))
      UNION ALL
      (SELECT AS STRUCT key, value, "system_label" AS x_type, FALSE AS x_inherited, "n/a" AS x_namespace FROM UNNEST(system_labels))
      UNION ALL
      (SELECT AS STRUCT key, value, "project_label" AS x_type, TRUE AS x_inherited, "n/a" AS x_namespace FROM UNNEST(project.labels))
      UNION ALL
      (SELECT AS STRUCT key, value, "tag" AS x_type, inherited AS x_inherited, namespace AS x_namespace FROM UNNEST(tags))
    ) AS focus_tags
  FROM `project.dataset.gcp_billing_export_resource_v1_XXXXXX`  -- TODO: replace with your billing export table
),
...
prices AS (
  SELECT
    *,
    flattened_prices
  FROM `project.dataset.cloud_pricing_export`,  -- TODO: replace with your pricing export table
  UNNEST(list_price.tiered_rates) AS flattened_prices
  WHERE DATE(export_time) = 'YYYY-MM-DD'  -- TODO: replace with a date after you enabled pricing export
)
...
SELECT
  ...
  "TODO_BILLING_ACCOUNT_ID" AS BillingAccountId,  -- TODO: replace with billing account ID from table name
  ...
  (SELECT name FROM region_names WHERE id = usage_cost_data.location.region) AS RegionName,
...
1. Run the query in BigQuery
2. Click **Save** → **Save view**
3. Select project and dataset
4. Enter view name (e.g., `gcp_focus_view`)

## Limitations

- **ChargeCategory**: Only uses "usage", "tax", and "adjustment" (not "purchase" or "credit" separately)
- **EffectiveCost**: Calculated as cost + credits per line item (not moved from commitment SKU to usage)
- **BillingAccountId vs SubAccountId**: For reseller accounts, BillingAccountId is the parent, SubAccountId is the resold account
- **PricingCategory**: Doesn't support "dynamic" value
- **ServiceCategory**: Uses GCP product taxonomy, not exact FOCUS values
- **ChargeClass**: Shows "correction" for all adjustments
- **CommitmentDiscountId**: Not always present
- **ListUnitPrice & ServiceCategory**: Use price as of the date in the query, not the date of usage
